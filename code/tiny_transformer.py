...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if encoder_output is None:
            ...
            # 交叉注意力：需要独立的K、V投影（因为K、V来自编码器，Q来自解码器）
            k = self.k_proj_cross(encoder_output).view(B,T_enc,self.n_head,C//self.n_head).transpose(1,2)
            v = self.v_proj_cross(encoder_output).view(B,T_enc,self.n_head,C//self.n_head).transpose(1,2)
